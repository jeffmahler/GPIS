function [class_results, path_lengths, class_losses, class_images] = ...
    discrete_gp_ls_subsample(f_grid, config, start_ind)
...
step_size_w = (width / sqrt(num_iters));
step_size_h = (height / sqrt(num_iters));

indices = zeros(num_iters, 1);
index = 1;
col_count = 1;
for i = 1:step_size_w:height
    for j = 1:step_size_h:width
        if mod(col_count, 2) == 0
            j_ind = width - j;
        else
            j_ind = j;
        end
        indices(index) = ...
            round(sub2ind([height, width], ...
                min(height, max(1, round(i))), ...
                min(width, max(1, round(j_ind)))));
        index = index + 1;
    end
    col_count = col_count + 1;
end
indices(index:end) = ...
    max(1, round(sub2ind([height, width], round(i), round(j_ind))));
