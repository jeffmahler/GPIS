...
experiment_config.num_function_samples = 10;
...
lse_config.path_penalty = 1e-1;
lse_config.beam_size = 1;
lse_config.horizon = 1;
lse_config.use_dec_path_penalty = 1;
...
lse_config.min_rot = 0;
lse_config.max_rot = 2 * pi;
lse_config.min_trans = [-lse_config.height / 8, -lse_config.width / 8];
lse_config.max_trans = [lse_config.height / 8, lse_config.width / 8];
lse_config.min_scale = 0.9;
lse_config.max_scale = 1.1;
lse_config.resample_size = lse_config.num_pose_particles;
lse_config.gaussian_resample = 1;
...
%% analyze results
[avg_u, avg_f, avg_p, labels] = analyze_lse_pose_results(experiment_results, experiment_config);
