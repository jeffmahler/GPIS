...
bb_centroid = (max(surf_points) + min(surf_points)) / 2;
    ...
    if vis
        ...
        for j = 1:num_faces
            line_f = [contact(1), contact(2), contact(3);
                      contact + cone_support_x1(:,j)' / norm(cone_support_x1(:,j))];
    ...
    for j = 1:num_samples
        ...
        for k = 1:num_dirs
            ...
            [contacts, contact_found] = ...
                antipodal_grasp_contacts(x1, v, sdf, grasp_width, config.surf_thresh);
            if ~contact_found
                continue;
            end
            x2 = contacts(2,:);
            ...
            if vis
                num_faces = size(cone_support_x2, 2);
                contact2 = x2;%[0, 0, 0];
                figure(2);
                %clf;
                for z = 1:num_faces
                    line_f = [contact2(1), contact2(2), contact2(3);
                              contact2 + cone_support_x2(:,z)' / norm(cone_support_x2(:,z))];
                    plot3(line_f(:,1), line_f(:,2), line_f(:,3), 'g', 'LineWidth', 5);
                    hold on;
                end
            end
            ...
            if in_cone1 && in_cone2
                ...
                if alpha1 < alpha_thresh && alpha2 < alpha_thresh && ...
                    ...
                    [R_g_obj_list, t_g_obj_list, g1_obj, g2_obj] = ...
                        grasp_points_to_poses(grasp, bb_centroid, sdf_res, config);
