...
trans_var_orig = config.pose_var(1:2,1:2); 
for i = 1:config.num_iters
    close all; 
    config.pose_var(1:2,1:2) = i*trans_var_orig; 
...
clf;
hold on; 
%plot(degs,avg_random_simp_convg, 'b', 'LineWidth', 3);
%plot(avg_ucb_simp_convg, 'r', 'LineWidth', 3);c
plot(degs,avg_thomp_simp_convg, 'r', 'LineWidth', 3);
...
title('Sensitivity Analysis of Translational Variance', 'FontSize', 15);
    %axis([1 config.num_iters 0 25e3]);   
                               
hold off; 
save('results/bandits/results_trans_var.mat', 'results_fric_var','-v7.3');
