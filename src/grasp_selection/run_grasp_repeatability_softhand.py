...
import mab_single_object_softhand_objective as msoo
...
if __name__ == '__main__':
    ...
    for grasp in grasps:
        if grasp.grasp_id in grasp_ids and grasp.grasp_id >= config['start_grasp_id']:
            grasps_to_execute.append(grasp.parallel_table(stable_pose))
            if len(grasps_to_execute) >= 20:
                break
    ...
    if True:#try:
        ...
        objective = msoo.MABSingleObjectSoftHandObjective(graspable, stable_pose, ds, ctrl, camera,
