...
if __name__ == '__main__':
    ...
    for k in range(config['num_avg']):
        ...
        # average the results
        R_tfx = tfx.rotation(R)
        R_new = tfx.rotation(rotation_camera_world)
        R_tfx = R_tfx.interpolate(R_new, 1.0 / (k+1))
        R = np.array(R_tfx.matrix)
        t = (k * t + translation) / (k + 1)

    rotation_camera_world = R
    translation = t
    print('Final Result')
    print('Rotation: ')
    print(rotation_camera_world)
