...
class FeatureObjectDatabase:
	...
	def _create_feature_objects_with_blobs(self, final_blobs, rendered_objects, vector_merging_method):
		feature_objects = {}
		index = 0
		for i, rendered_object in enumerate(rendered_objects):
			if i%int(len(rendered_objects)/20+1) is 0:
	...
	def _create_feature_objects_with_database(self, feature_database, net, batch_size):
		...
		feature_objects = self._create_feature_objects_with_blobs(final_blobs, rendered_objects, caffe_config.vector_merging_method)
...
if __name__ == '__main__':
	...
	feature_db.save_feature_object_database(feature_object_db)
