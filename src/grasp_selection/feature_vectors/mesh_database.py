	...
	def create_categories(self, path_to_index):
		...
		with open(path_to_index) as file:
			for line in file:
				...
				file_id = split[0]
				category = split[1]
				if file_id in object_dict:
					object_dict[file_id].append(category)
				else:
					object_dict[file_id] = [category]
		self.object_dict_ = object_dict

class SHRECObjectDatabase(MeshDatabase):
	def __init__(self, path_to_index):
		self.categories_, self.object_keys_ = self.create_categories(path_to_index)
		...
		with open(path_to_index) as file:
			...
			for line in file:
				...
				if len(split) == 3:
					...
					object_dict[object_key] = current_class
		return categories, object_keys
