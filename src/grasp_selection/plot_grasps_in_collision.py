...
if __name__ == '__main__':
    ...
    for graspable, masked_graspables, mask in zip(objects, masked_objects, masks):
        ...
        public_coll_free_grasp_ids = [g.grasp_id for g in public_coll_free_grasps]

        masked_public_coll_free_grasps, masked_public_coll_free_qualities = \
            coverage.prune_grasps_in_collision(masked_graspables[0], public_grasps, public_qualities,
                                               gripper, most_stable_pose, config)

        mlab.clf()
        T_obj_world = mv.MayaviVisualizer.plot_stable_pose(graspable.mesh, most_stable_pose, d=0.15)        

        for masked_grasp, masked_quality in zip(masked_public_coll_free_grasps, masked_public_coll_free_qualities):
            if masked_grasp.grasp_id not in public_coll_free_grasp_ids:
                mlab.clf()
                coverage.vis_coverage(public_mesh, [private_mesh], [masked_grasp], [masked_quality],
                                      most_stable_pose, config, gripper=gripper, color_gripper=False,
                ...
                for j in range(num_views):
                    ...
                    figname = 'collision_grasp_%d_%s_gripper_view_%d.png' %(masked_grasp.grasp_id, obj_name, j)
