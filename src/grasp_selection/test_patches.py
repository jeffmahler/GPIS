...
def set_mayavi_scene_for_contacts(obj, grasp, c1, c2, style='wireframe',
                                  opacity=1.0):
    alpha = 0.025
    tube_radius = 0.001
    scale = 0.0025
    ...
    mvis.plot_mesh(obj.mesh, T_obj_world, style=style, opacity=opacity)
    mvis.plot_grasp(grasp, T_obj_world, alpha=alpha, tube_radius=0.0005, endpoint_scale=0.00025)
...
def visualize_patch_mesh(T_c1_world, T_c2_world, obj_key, grasp_num, 
                       width, num_steps, w1, w2, output=None):
    res = float(width) / float(num_steps)
    scale = 0.0025
    mesh_background1, mesh_tris1, points1 = \
        mvis.plot_patch(w1, T_c1_world, window_dim_obj=(width, width), patch_color=(1,0,1))
    mesh_background2, mesh_tris2, points2 = \
        mvis.plot_patch(w2, T_c2_world, window_dim_obj=(width, width), patch_color=(0,1,1))
    ...
    if output is None:
        ...
        for i in range(num_views):
            az = i * delta_view
            mv.view(azimuth=az, focalpoint=(0,0,0), distance=cam_dist)
            mv.savefig(os.path.join(output, "contacts_mesh_{0}_grasp_{1}_{2}.png".format(obj_key, grasp_num, i)))

    return mesh_background1, mesh_tris1, points1, mesh_background2, mesh_tris2, points2
...
def test_view_mesh_patches(dataset, config, args):
    gripper_name = config['gripper']
    width = float(config['width'])
    num_steps = int(config['num_steps'])
    sigma_range = float(config['sigma_range'])
    ...
    samples_per_grid = int(config['samples_per_grid'])
    back_up = float(config['back_up'])
    max_projection = float(config['max_projection'])
    opacity = 0.95
        ...
        if interactive:
            ...
            while True:
                ...
                if True:#try:
                    pre_blur = []
                    w1, w2, c1, c2 = obj.surface_information(grasp, width, num_steps, samples_per_grid=samples_per_grid,
                                                             back_up=back_up, max_projection=max_projection, sigma_range=sigma_range, 
                                                             sigma_spatial=sigma_spatial, debug_objs=pre_blur)
                    logging.info("Visualizing patches")
                    mv.clf()
                    T_c1_world, T_c2_world = set_mayavi_scene_for_contacts(obj, grasp, c1, c2, style='surface',
                                                                           opacity=opacity)

                    visualize_patches(w1, w2, pre_blur[0], pre_blur[1], obj_key, i, settings)
                    mesh_bgrnd1, mesh_tris1, points1, mesh_bgrnd2, mesh_tris2, points2 = \
                        visualize_patch_mesh(T_c1_world, T_c2_world, obj_key, i, width, num_steps, w1, w2)
                    mesh_bgrnd1.remove()
                    mesh_tris1.remove()
                    points1.remove()
                    mesh_bgrnd2.remove()
...
if __name__ == '__main__':
    ...
    if args.mode == 'all':
        ...
        test_view_mesh_patches(dataset, config, args)
